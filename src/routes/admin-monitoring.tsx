    ...
    <div className="space-y-6 max-w-6xl mx-auto pb-10">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">Real-time health tracking and database monitoring.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        {[
          { label: "System Uptime", value: "99.98%", icon: Clock, color: "text-emerald-500", bg: "bg-emerald-100 dark:bg-emerald-900/30" },
          { label: "Server Load", value: "24%", icon: Cpu, color: "text-blue-500", bg: "bg-blue-100 dark:bg-blue-900/30" },
          { label: "Storage Used", value: "45 GB", icon: HardDrive, color: "text-purple-500", bg: "bg-purple-100 dark:bg-purple-900/30" },
          { label: "Active Connections", value: "1,240", icon: Server, color: "text-orange-500", bg: "bg-orange-100 dark:bg-orange-900/30" },
        ].map((stat, i) => (
            ...
            {[
              ...
              <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 border border-slate-100 dark:border-slate-800 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/50">
                <div className="mb-2 sm:mb-0">
                  ...
                  <p className="text-xs text-slate-500">Latency: {db.ping} • {db.storage}</p>
                </div>
                <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                  db.status === 'Healthy' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400'
                }`}>
                  {db.status}
                </span>
              </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
          ...
          <div className="space-y-4 h-[300px] overflow-y-auto pr-2">
            {[
              ...
              { log: "Failed login attempt from IP 192.168.1.1", time: "1 hour ago", type: "warning" },
              { log: "Database backup completed", time: "3 hours ago", type: "success" },
              { log: "API Rate limit approaching for /predict endpoint", time: "5 hours ago", type: "warning" },
              { log: "System updated to v2.1.0", time: "1 day ago", type: "info" },
            ].map((log, i) => (
              <div key={i} className="flex gap-4 p-3 rounded-lg border border-transparent hover:border-slate-100 dark:hover:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/30">
                ...
                <div>
                  <p className="text-sm text-slate-800 dark:text-slate-200 font-medium">{log.log}</p>
