...
const sections = [
  {
    ...
    questions: [
      ...
      { id: 'q4', text: 'Highest level of education', type: 'single', options: ['Primary', 'Secondary', 'Graduate', 'Post-graduate'] },
    ],
  },
  {
    id: 'B', title: 'Oral Hygiene Practices',
    questions: [
      { id: 'q5', text: 'How many times do you brush your teeth in a day?', type: 'single', options: ['Once', 'Twice', 'More than twice', 'Irregular'] },
      { id: 'q6', text: 'Do you use fluoridated toothpaste?', type: 'single', options: ['Yes', 'No', "Don't know"] },
      { id: 'q7', text: 'Do you use any of the following? (select all that apply)', type: 'multi', options: ['Dental floss', 'Interdental brush', 'Mouthwash', 'None'] },
      { id: 'q8', text: 'How often do you clean your tongue?', type: 'single', options: ['Daily', 'Occasionally', 'Never'] },
    ],
  },
  {
    id: 'C', title: 'Dietary Habits',
    questions: [
      { id: 'q9', text: 'How often do you consume sugary foods or drinks?', type: 'single', options: ['Rarely', 'Once a day', '2–3 times a day', 'More than 3 times a day'] },
  ...
  {
    id: 'D', title: 'Previous Dental History',
    questions: [
      { id: 'q12', text: 'Have you had tooth decay diagnosed?', type: 'single', options: ['Yes', 'No', 'Not sure'] },
      { id: 'q13', text: 'Have you had gum problems?', type: 'yesno' },
      { id: 'q14', text: 'Previous dental treatments (select all that apply)', type: 'multi', options: ['Fillings', 'Root canal', 'Extraction', 'Scaling', 'None'] },
      { id: 'q15', text: 'How often do you visit the dentist?', type: 'single', options: ['Only when in pain', 'Once a year', 'Every 6 months', 'Never'] },
    ],
  },
  {
    id: 'E', title: 'Current Oral Symptoms',
    questions: [
  ...
  {
    id: 'F', title: 'General Health',
    questions: [
      { id: 'q21', text: 'Do you have any of these conditions? (select all that apply)', type: 'multi', options: ['Diabetes', 'Asthma', 'Thyroid', 'Heart disease', 'None', 'Others'] },
      { id: 'q22', text: 'Are you on long-term medications?', type: 'yesno' },
    ],
  },
  {
    id: 'G', title: 'Lifestyle',
    questions: [
  ...
  {
    id: 'H', title: 'Awareness',
    questions: [
      { id: 'q25', text: 'Can dental problems be prevented if detected early?', type: 'single', options: ['Yes', 'No', 'Not sure'] },
      { id: 'q26', text: 'Willing to follow preventive advice from the app?', type: 'single', options: ['Yes', 'No', 'Maybe'] },
    ],
  },
  {
    id: 'I', title: 'App Acceptance',
    questions: [
  ...
  {
    id: 'J', title: 'Self-Perceived Oral Health',
    questions: [
      { id: 'q30', text: 'How would you rate your overall oral health?', type: 'single', options: ['Very good', 'Good', 'Fair', 'Poor'] },
    ],
  },
];
...
function computeRisk(answers: Record<string, any>) {
  ...
  // Section C – Diet
  score += ({ 'Rarely': 0, 'Once a day': 5, '2–3 times a day': 10, 'More than 3 times a day': 15 } as any)[answers.q9] ?? 0;
  ...
  score += ({ 'Only when in pain': 10, 'Once a year': 3, 'Every 6 months': 0, 'Never': 15 } as any)[answers.q15] ?? 0;
  ...
  const breakdown = [
    ...
    { label: 'Diet', value: Math.min(100, Math.round((((({ 'Rarely': 0, 'Once a day': 5, '2–3 times a day': 10, 'More than 3 times a day': 15 } as any)[answers.q9] ?? 0) + (answers.q10 === 'Yes' ? 5 : 0) + (answers.q11 === 'Yes' ? 8 : 0)) / 28) * 100)), color: '#FFCDB2' },
    { label: 'Symptoms', value: Math.min(100, Math.round(((answers.q16 === 'Yes' ? 10 : 0) + (answers.q17 === 'Yes' ? 10 : 0) + (answers.q18 === 'Yes' ? 12 : 0) + (answers.q19 === 'Yes' ? 6 : 0) + (answers.q20 === 'Yes' ? 15 : 0)) / 53 * 100)), color: '#EF4444' },
    { label: 'Lifestyle', value: Math.min(100, Math.round(((answers.q23 === 'Yes' ? 12 : 0) + (answers.q24 === 'Yes' ? 5 : 0)) / 17 * 100)), color: '#7C3AED' },
  ];
  ...
  const recommendations = level === 'High'
    ? ['Visit a dentist within the next 2 weeks', 'Brush twice daily with fluoride toothpaste', 'Stop tobacco and limit alcohol', 'Start flossing daily']
    : level === 'Medium'
    ? ['Schedule a dental visit within 3 months', 'Reduce sugary food consumption', 'Brush twice daily and floss regularly', 'Consider using mouthwash']
    : ['Maintain your current hygiene routine', 'Continue 6-monthly dental check-ups', 'Stay hydrated and limit sugary drinks'];
...
export default function AssessmentScreen() {
  ...
  const toggleMulti = (qId: string, option: string) => {
    ...
    if (without.includes(option)) {
      setAnswer(qId, without.filter(o => o !== option));
    } else {
    ...
    <PhoneShell>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1, marginLeft: 12 }}>
          <Text style={styles.headerLabel}>Section {section.id} of {sections.length}</Text>
      ...
      <View style={styles.progressBg}>
        <View style={[styles.progressFill, { width: `${((sectionIndex + 1) / sections.length) * 100}%` as any }]} />
      </View>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
            ...
            {q.type === 'yesno' && (
              <View style={styles.yesnoRow}>
                {['Yes', 'No'].map(opt => (
                  <TouchableOpacity
                    key={opt}
                    style={[styles.yesnoBtn, answers[q.id] === opt && (opt === 'Yes' ? styles.yesnoBtnYes : styles.yesnoBtnNo)]}
                    onPress={() => setAnswer(q.id, opt)}
                  >
                    <Feather name={opt === 'Yes' ? 'check' : 'x'} size={16} color={answers[q.id] === opt ? '#FFFFFF' : '#64748B'} />
                    <Text style={[styles.yesnoBtnText, answers[q.id] === opt && { color: '#FFFFFF' }]}>{opt}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            )}
        ...
        >
          {saving ? (
            ...
            <>
              <Text style={styles.nextBtnText}>{isLast ? 'Submit Assessment' : 'Next Section'}</Text>
...
const styles = StyleSheet.create({
  ...
  headerLabel: { fontSize: 11, color: '#64748B', fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.5 },
  headerTitle: { fontSize: 16, fontWeight: '700', color: '#0F172A', marginTop: 2 },
  sectionBadge: { width: 40, height: 40, borderRadius: 14, backgroundColor: '#86F1D4', alignItems: 'center', justifyContent: 'center' },
  sectionBadgeText: { fontSize: 16, fontWeight: '800', color: '#0D4B42' },
  progressBg: { height: 5, backgroundColor: '#E2E8F0', marginHorizontal: 20, borderRadius: 3, overflow: 'hidden', marginBottom: 4 },
  ...
  optionsList: { gap: 8 },
  optionBtn: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 12, paddingHorizontal: 14, borderRadius: 14, borderWidth: 1.5, borderColor: '#E2E8F0', backgroundColor: '#F8FAFC' },
  ...
  nextBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, backgroundColor: '#86F1D4', paddingVertical: 18, borderRadius: 18, marginTop: 8, elevation: 4, shadowColor: '#86F1D4', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8 },
