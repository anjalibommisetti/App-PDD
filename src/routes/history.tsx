      ...
      if (tab === 'assessments') {
        ...
        if (data && data.length > 0) {
          setItems(data.map((it: any) => {
            ...
            const tone = isInProgress ? 'pending'
              : it.level === 'High' ? 'alert'
              : it.level === 'Medium' ? 'warning' : 'success';
            return {
              ...it,
              type: 'assessment',
              isScan,
            ...
            return (
              <View key={it.id} style={styles.itemCard}>
                ...
                <View style={styles.itemBody}>
                  ...
                  <Text style={styles.itemDate}>{it.displayDate} · {it.displayTime}</Text>
                  {it.note ? <Text style={styles.itemNote} numberOfLines={1}>📝 {it.note}</Text> : null}
                  <View style={[styles.badge, { backgroundColor: c.bg, marginTop: 4 }]}>
                    <Text style={[styles.badgeText, { color: c.fg, textTransform: 'capitalize' }]}>{it.status || 'pending'}</Text>
                  </View>
                </View>
                <TouchableOpacity
                  style={styles.deleteBtn}
...
const styles = StyleSheet.create({
  ...
  tabsWrap: {
    ...
    backgroundColor: '#F1F5F9', padding: 6, borderRadius: 16, marginBottom: 12,
  },
  tabBtn: {
  ...
  actionBtn: {
    marginTop: 8, backgroundColor: '#86F1D4',
