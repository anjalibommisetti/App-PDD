...
(async () => {
  const browser = await puppeteer.launch({ executablePath: 'C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe', headless: true, defaultViewport: { width: 1280, height: 800 } });
  ...
  await page.evaluate(() => {
    const splashBtn = Array.from(document.querySelectorAll('div, text, span')).find(el => el.innerText && el.innerText.includes('Get Started'));
